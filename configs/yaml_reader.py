...
class AbstractYAMLReader(metaclass=ABCMeta):
    ...
    def __init__(self, config_dict):
        ...
        print('XXXXXXXXXXXXXXXXX',self.file_patterns)
...
class GenericYAMLReader(AbstractYAMLReader):
    ...
    def filename_items_for_filetype(filenames, filetype_info):
        ...
        print(f"🟢 文件名：{filenames},文件类型信息: { filetype_info } ") # 关键日志
        if not isinstance(filenames, set):
            # we perform set operations later on to improve performance
            filenames = set(filenames)
        for pattern in filetype_info["file_patterns"]:
            ...
            for filename in matches:
                try:
                    ...
                    print(f"✅ 解析成功: {filename} -> {filename_info}")
                except ValueError as e:
                    logger.debug("Can't parse %s with %s.", filename, pattern)
                    print(f"❌ 解析失败: {filename} (错误: {e})") # 关键日志
                    continue
...
class FileYAMLReader(GenericYAMLReader, DataDownloadMixin):
    ...
    #     for filename, filename_info in filename_items:
    #         try:
    #             print(f"🔄 尝试创建文件处理器: {filename}")
    #             req_fh = self.find_required_filehandlers(requirements,
    #                                                      filename_info)
    #             print(f"🟢 创建文件处理器成功: {filename}")
    ...
    def _new_filehandler_instances(self, filetype_info, filename_items, fh_kwargs=None):
        ...
        for filename, filename_info in filename_items:
            try:
                print(f"🔄 尝试创建文件处理器: {filename}")
                req_fh = self.find_required_filehandlers(requirements, filename_info)
                handler = filetype_cls(filename, filename_info, filetype_info, *req_fh, **fh_kwargs)
                print(f"✅ 文件处理器创建成功: {filename}")
                yield handler
            except Exception as e:
                print(f"❌ 文件处理器创建失败: {filename} (错误: {str(e)})")
                continue
    ...
    #                                                      filetype_info)
    #     filename_list = []
    #     for filename, filename_info in filename_iter:
    #         print(f"🔍 文件名解析结果: {filename_info}")
    #         filename_list.append((filename, filename_info))
    #     print(f"🟢 所有解析结果: {filename_list}")

    #     # 重新创建生成器
    #     filename_iter = iter(filename_list)
    ...
    def _new_filehandlers_for_filetype(self, filetype_info, filenames, fh_kwargs=None):
        ...
        filename_iter = self.filename_items_for_filetype(filenames,
                                                         filetype_info)
        if self.filter_filenames:
            print(f"yy {self.filter_filenames}, 🔄 过滤文件名")
            # preliminary filter of filenames based on start/end time
            # to reduce the number of files to open
            filename_iter = self.filter_filenames_by_info(filename_iter)
        print(f"zz {filename_iter}, 🔄 文件名迭代器")
        filehandler_iter = self._new_filehandler_instances(filetype_info,
                                                           filename_iter,
                                                           fh_kwargs=fh_kwargs)
        print(f"aa {filehandler_iter}, 🔄 文件处理器迭代器")
        filtered_iter = self.filter_fh_by_metadata(filehandler_iter)
        return list(filtered_iter)
    ...
    def create_filehandlers(self, filenames, fh_kwargs=None):
        ...
        for filetype, filetype_info in self.sorted_filetype_items():
            print(f"🟢 创建文件处理器，文件类型: { filetype }, 文件类型信息: { filetype_info }")
            filehandlers = self._new_filehandlers_for_filetype(filetype_info,
                                                               filename_set,
                                                               fh_kwargs=fh_kwargs)
            print(f"🟢 创建文件处理器，文件类型: { filetype }, 文件处理器: { filehandlers }")
            if filehandlers:
